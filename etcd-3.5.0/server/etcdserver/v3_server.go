...
// 经过一系列鉴权，然后通过propose向raft模块提交一个json提案
func (s *EtcdServer) processInternalRaftRequestOnce(ctx context.Context, r pb.InternalRaftRequest) (*applyResult, error) {
	// 请求限速：提交的减去应用的大于5000就限速。
	...
	// 检查如果携带token，校验token的合法性  check authinfo if it is not InternalAuthenticateRequest
	...
	// 包大小超出默认DefaultMaxRequestBytes=1M值就返回
	if len(data) > int(s.Cfg.MaxRequestBytes) {
		return nil, ErrRequestTooLarge
	}

	// 将消息绑定一个管道，并将管道注册到待发送列表，raft会将结果写入管道
	id := r.ID
	if id == 0 {
		id = r.Header.ID
	}
	...
	// 等待返回结果处理成applyResult结构体，或获取错误并返回
	select {
	case x := <-ch:
