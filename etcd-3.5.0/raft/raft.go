...
// 存储要发送到集群各个节点的消息，待发送 send schedules persisting state to a stable storage and AFTER that
...
func (r *raft) advance(rd Ready) {
	...
	// 先存到unstable存储区中，存的是索引位置。（等raftlog模块再接收到要持久化的命令时，直接日志数据再存到raftlog文件中。）
	if len(rd.Entries) > 0 {
...
// 参加选举，将节点从follower变为candidate状态
...
func (r *raft) Step(m pb.Message) error {
	// 通过比较消息和当前节点的任期号处理消息 Handle the message term, which may result in our stepping down to a follower.
	switch {
	case m.Term == 0:
		// 如果消息任期号是0，表示是本地消息  local message
	case m.Term > r.Term:
		// 消息任期号比当前节点任期号大，说明当前节点保存的leader任期号是过期了的。
		// 如果是投票消息，但不是竞选的上下文，并且当前节点不是leader节点，选举计数还未超时，则直接忽略此次投票。
		...
		case m.Type == pb.MsgPreVote:
			// 收到的第一阶段发起的投票选举 Never change our term in response to a PreVote
		case m.Type == pb.MsgPreVoteResp && !m.Reject:
			// 第一阶段发起的投票选举的收到的响应结果，并且投票没有被拒绝 We send pre-vote requests with a term in our future. If the
			// pre-vote is granted, we will increment our term when we get a
			// quorum. If it is not, the term comes from the node that
			// rejected our vote so we should become a follower at the new
			// term.
		default:
			// 当前节点任期号过期，处理消息，需要变成follower再去处理
			r.logger.Infof("%x [term: %d] received a %s message with higher term from %x [term: %d]",
				r.id, r.Term, m.Type, m.From, m.Term)
			if m.Type == pb.MsgApp || m.Type == pb.MsgHeartbeat || m.Type == pb.MsgSnap {
				// follower需要使用leader的信息处理快照 心跳 的消息
				r.becomeFollower(m.Term, m.From)
			} else {
				r.becomeFollower(m.Term, None)
			}
		}
	// 消息任期号小于当前节点任期号，消息过期了
	case m.Term < r.Term:
...
func stepFollower(r *raft, m pb.Message) error {
	...
	case pb.MsgProp:
		...
		m.To = r.lead
		r.send(m) // 发送给leader去处理，stepleader
	case pb.MsgApp:
		r.electionElapsed = 0
		r.lead = m.From
		r.handleAppendEntries(m)
...
// entry at a new leader's term, as well as leaving a joint configuration.// Copyright 2015 The etcd Authors
////
//// Licensed under the Apache License, Version 2.0 (the "License");
//// you may not use this file except in compliance with the License.
//// You may obtain a copy of the License at
